...
from fastapi import BackgroundTasks, Body, Depends, FastAPI, Header, HTTPException, Request  # noqa: F401 — used by routers/* via main.<name> (E0 late-binding contract)
from fastapi.concurrency import run_in_threadpool  # noqa: F401 — used by routers/* via main.<name> (E0 late-binding contract)
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import StreamingResponse  # noqa: F401 — used by routers/* via main.<name> (E0 late-binding contract)
from fastapi.security import HTTPAuthorizationCredentials, HTTPBearer
from pydantic import BaseModel, Field
from pydantic_settings import BaseSettings

from nexus import ai_planner as nexus_ai_planner
from nexus import bookings as nexus_bookings  # noqa: F401 — used by routers/* via main.<name> (E0 late-binding contract)
from nexus import copilot as nexus_copilot  # noqa: F401 — used by routers/* via main.<name> (E0 late-binding contract)
from nexus import db as nexus_db
from nexus import dossier as nexus_dossier  # noqa: F401 — used by routers/* via main.<name> (E0 late-binding contract)
...
from nexus import work_queue as nexus_work_queue  # noqa: F401 — used by routers/* via main.<name> (E0 late-binding contract)
...
from nexus.flows import authoring as nexus_flows_authoring  # noqa: F401 — used by routers/* via main.<name> (E0 late-binding contract)
...
from nexus.flows import simulate as nexus_flows_simulate  # noqa: F401 — used by routers/* via main.<name> (E0 late-binding contract)
...
_DEFAULT_CONFIG = {
    ...
    "whatsapp.insight_instructions": (
        "האדם שיתף איתך מה עובר עליו. המשימה שלך: להגיב במשפט אחד עד שניים בלבד, "
        "בגוף שני ובטון עממי — בלי לדבר במקומו ובלי גוף ראשון ('אני מרגיש…' אסור).\n\n"
        "⚠️ התאמת עומק — הכלל הכי חשוב: התאם את עומק התשובה לעומק של מה שנכתב.\n"
        "- קלט קצר, שטחי, או תלונה כללית בלי הקשר רגשי עמוק ← אל תמציא קונפליקט "
        "נפשי, טראומה או 'מה שקיווה שיהיה'. פשוט תקֵף את התסכול בצורה הגיונית, "
        "ישירה וקצרה מאוד (למשל: 'לגמרי, זה מתסכל שאי אפשר פשוט לדבר ישר ולעניין').\n"
        "- רק כשהאדם משתף משהו אישי ועמוק באמת ← שקף את הקונפליקט הפנימי שלו לפי "
        "אחד משלושת הצירים למטה.\n\n"
        "חוקי ברזל:\n"
        ...
        "שלושת הצירים (רק לקלט עמוק; בחר אחד, המדויק ביותר):\n"
        ...
        "מה שקיווה שיקרה.\n\n"
        "דוגמאות (אל תעתיק — תייצר חדש לפי מה שנכתב):\n"
        "• קלט שטחי: 'הבנות היום הזויות בתקשורת' ← 'לגמרי, זה מתסכל שאי אפשר פשוט "
        "לדבר ישר ולעניין.'\n"
        "• קלט עמוק: 'נראה שאתה מבין בשכל מה נכון, אבל הרגש עדיין לא שם.'\n"
        "• קלט עמוק: 'נשמע שהכי מתיש אותך זה שאתה בלופ ולא מצליח לצאת ממנו.'"
    ),
...
def _resolve_default_channel(messages: list) -> str:
    """
    One Thread §4.1 — 'reply to last inbound', locked 2026-07-09: the channel
    of the person's most recent LEAD-authored message (role='user'; 'assistant'
    is our own bot handoff ack, not theirs). Falls back to the last message of
    any role, then to 'whatsapp' for a brand-new/empty thread.
    """
    for m in reversed(messages):
        if m.get("role") == "user" and m.get("channel"):
            return m["channel"]
    if messages and messages[-1].get("channel"):
        return messages[-1]["channel"]
    return "whatsapp"
...
def _channel_send_eligibility(conn, person_id: str, channel: str) -> dict:
    ...
    return {
        "eligible": eligible,
        "reason": None if eligible else "window_expired",
...
def route_and_send(conn, person_id: str, channel: str, text: str, *, by: str,
    ...
    if row is None and client_token:
        # Lost the race to a concurrent identical request — return its result
        # rather than silently reporting success with no row of our own.
        with conn.cursor() as cur:
            cur.execute(
...
# ── Routers (E0 strangler extraction) ─────────────────────────────────────────
# Mounted at the very bottom so every helper above is already defined when the
# router modules late-import main. Route bodies live in routers/; helpers stay
# here until they migrate into nexus/ (the strangler continues in later PRs).
from routers.cockpit import router as _cockpit_router    # noqa: E402
from routers.webhooks import router as _webhooks_router  # noqa: E402
from routers.flows import router as _flows_router        # noqa: E402
